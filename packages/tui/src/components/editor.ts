...
export class Editor implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		for (let i = 0; i < visibleLines.length; i++) {
			...
			const linePrefix = this.scrollOffset + i === 0 ? this.prompt : " ".repeat(promptWidth);
			...
			if (layoutLine.hasCursor && layoutLine.cursorPos !== undefined) {
				...
				if (after.length > 0) {
					...
					if (visibleWidth(linePrefix) + lineVisibleWidth > contentWidth && paddingX > 0) {
			...
			const lineWidthWithPrefix = visibleWidth(linePrefix) + lineVisibleWidth;
			const padding = " ".repeat(Math.max(0, contentWidth - lineWidthWithPrefix));
