...
class SessionList implements Component, Focusable {
	...
	private maxVisible: number = 6; // Max sessions visible (each item renders as two lines)
	...
	private filterSessions(query: string): void {
		...
		const nonEmptySessions = this.allSessions.filter((session) => session.messageCount > 0);
		const nameFiltered =
			this.nameFilter === "all" ? nonEmptySessions : nonEmptySessions.filter((session) => hasSessionName(session));
	...
	render(width: number): string[] {
		...
		// Render visible sessions as two-line items
		for (let i = startIndex; i < endIndex; i++) {
			...
			const titlePrefix = (isSelected ? theme.fg("accent", "› ") : "  ") + theme.fg("dim", prefix);
			const titleWidth = Math.max(10, width - visibleWidth(titlePrefix));
			const truncatedTitle = truncateToWidth(normalizedMessage, titleWidth, "…");

			let titleColor: "error" | "warning" | "accent" | null = null;
			...
			let styledTitle = titleColor ? theme.fg(titleColor, truncatedTitle) : truncatedTitle;
			if (isSelected) {
				styledTitle = theme.bold(styledTitle);
			}

			const timestamp = theme.bold(
				theme.fg(
					isConfirmingDelete ? "error" : isCurrent ? "accent" : "accent",
					formatSessionTimestamp(session.modified),
				),
			);
			const metadataParts = [timestamp, theme.fg("muted", formatMessageCount(session.messageCount))];
			if (this.showCwd && session.cwd) {
				metadataParts.push(theme.fg("dim", shortenPath(session.cwd)));
			}
			if (this.showPath) {
				metadataParts.push(theme.fg("dim", shortenPath(session.path)));
			}
			const metadataLine = truncateToWidth(
				`  ${" ".repeat(prefixWidth)}${metadataParts.join(theme.fg("muted", " · "))}`,
				width,
				"…",
			);

			let titleLine = truncateToWidth(`${titlePrefix}${styledTitle}`, width, "…");
			const detailsLine = metadataLine;
			if (isSelected) {
				titleLine = theme.bg("selectedBg", titleLine.padEnd(width, " "));
			}
