...
function getGroupLabel(metadata: PathMetadata): string {
	...
	if (metadata.source === "auto") {
		return metadata.scope === "user" ? `User (${getAgentDir()}/)` : `Project (${CONFIG_DIR_NAME}/)`;
	}
	...
	private toggleTopLevelResource(item: ResourceItem, enabled: boolean): void {
		...
		if (scope === "project") {
			if (arrayKey === "extensions") {
				...
				this.settingsManager.setProjectCommandTemplatePaths(updated);
			...
			if (arrayKey === "extensions") {
				...
				this.settingsManager.setCommandTemplatePaths(updated);
			} else if (arrayKey === "themes") {
