		...
		const items: SettingItem[] = [
			...
			},
			{
				...
				submenu: (currentValue, done) =>
					new SelectSubmenu(
						"Reasoning Effort",
						"Select reasoning depth for reasoning-capable models",
						config.availableReasoningEfforts.map((level) => ({
							value: level,
							label: level,
							description: REASONING_EFFORT_DESCRIPTIONS[level],
						})),
						currentValue,
						(value) => {
							callbacks.onReasoningEffortChange(value as ReasoningEffort);
							done(value);
						},
						() => done(),
					),
			...
			(id, newValue) => {
				switch (id) {
					...
					case "reasoning-effort":
						callbacks.onReasoningEffortChange(newValue as ReasoningEffort);
						break;
					case "hide-thinking":
