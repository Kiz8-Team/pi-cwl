...
import { CONFIG_DIR_NAME, getAgentDir, getBinDir, getProjectAgentDir } from "./config.js";
...
/**
 * Migrate sessions from the global agent root into session directories.
 *
 * Bug in v0.30.0: Sessions were saved to the global agent root instead of
 * agent/sessions/<encoded-cwd>/. This migration moves them to the correct
 * location based on the cwd in their session header.
 *
...
function moveDirectoryContents(sourceDir: string, targetDir: string): void {
	...
	for (const name of readdirSync(sourceDir)) {
		...
		try {
			...
			// Skip entries that can't be migrated
		}
	}
...
export function migrateSessionsFromAgentRoot(): void {
	...
	for (const file of files) {
		try {
			...
			const correctDir = join(agentDir, "sessions", encodeCwdForSessionDir(cwd));
...
/**
 * Migrate project-local trace storage into the global agent dir.
 *
 * Sessions move from <cwd>/.claude/agent/sessions/<encoded-cwd>/ to
 * ~/.claude/agent/sessions/<encoded-cwd>/, and CWL traces move from
 * <cwd>/.claude/agent/cwl/ to ~/.claude/agent/cwl/.
 */
export function migrateProjectLocalTraces(cwd: string): void {
	const projectAgentDir = getProjectAgentDir(cwd);
	const globalAgentDir = getAgentDir();

	if (projectAgentDir === globalAgentDir) return;
	if (!existsSync(projectAgentDir)) return;

	const encodedCwd = encodeCwdForSessionDir(cwd);
	moveDirectoryContents(join(projectAgentDir, "sessions", encodedCwd), join(globalAgentDir, "sessions", encodedCwd));
	moveDirectoryContents(join(projectAgentDir, "cwl"), join(globalAgentDir, "cwl"));
}

/**
 * Migrate legacy prompts/ to commands/ if needed.
