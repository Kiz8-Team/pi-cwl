...
export class ExtensionRunner {
	...
	async emitResourcesDiscover(
		...
		commandPaths: Array<{ path: string; extensionPath: string }>;
		themePaths: Array<{ path: string; extensionPath: string }>;
	}> {
		const ctx = this.createContext();
		const skillPaths: Array<{ path: string; extensionPath: string }> = [];
		const commandPaths: Array<{ path: string; extensionPath: string }> = [];
		...
		for (const ext of this.extensions) {
			...
			for (const handler of handlers) {
				try {
					...
					if (result?.commandPaths?.length) {
						commandPaths.push(...result.commandPaths.map((path) => ({ path, extensionPath: ext.path })));
					}
		...
		return { skillPaths, commandPaths, themePaths };
	}
