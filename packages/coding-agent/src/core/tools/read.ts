...
function formatReadCall(
	...
	return `${bullet} ${theme.fg("toolTitle", theme.bold("Read"))}(${pathDisplay})`;
}
...
function formatReadResult(
	...
	const renderedBlock = displayLines
		.map((line) => (lang ? replaceTabs(line) : theme.fg("toolOutput", replaceTabs(line))))
		.join("\n");
	let text = indentToolBlock(renderedBlock);
	if (remaining > 0) {
	...
	if (truncation?.truncated) {
		if (truncation.firstLineExceedsLimit) {
			text += `\n${theme.fg("warning", "  ")}${theme.fg("warning", `[First line exceeds ${formatSize(truncation.maxBytes ?? DEFAULT_MAX_BYTES)} limit]`)}`;
		} else if (truncation.truncatedBy === "lines") {
			text += `\n${theme.fg("warning", "  ")}${theme.fg("warning", `[Truncated: showing ${truncation.outputLines} of ${truncation.totalLines} lines (${truncation.maxLines ?? DEFAULT_MAX_LINES} line limit)]`)}`;
		} else {
			text += `\n${theme.fg("warning", "  ")}${theme.fg("warning", `[Truncated: ${truncation.outputLines} lines shown (${formatSize(truncation.maxBytes ?? DEFAULT_MAX_BYTES)} limit)]`)}`;
		}
	}
	return text;
}
...
export function createReadToolDefinition(
	...
	return {
		...
		renderCall(args, theme, context) {
			...
			text.setText(formatReadCall(args, theme, context.isPartial, context.isError));
			return text;
		},
