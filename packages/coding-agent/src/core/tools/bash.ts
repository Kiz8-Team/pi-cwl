...
import { getTextOutput, getToolStatusBullet, indentToolBlock, invalidArgText, str } from "./render-utils.js";
...
function formatBashCall(
	...
	return `${bullet} ${theme.fg("toolTitle", theme.bold("Run"))}(${commandDisplay})${timeoutSuffix}`;
}
...
function rebuildBashResultRenderComponent(
	...
	if (output) {
		...
		if (options.expanded) {
			component.addChild(new Text(indentToolBlock(styledOutput), 0, 0));
		} else {
			component.addChild({
				render: (width: number) => {
					...
					if (state.cachedLines === undefined || state.cachedWidth !== width) {
						const preview = truncateToVisualLines(styledOutput, BASH_PREVIEW_LINES, width - 2);
					...
					if (state.cachedSkipped && state.cachedSkipped > 0) {
						...
						return [
							"",
							prefixLine(hint, theme.fg("muted", "⎿ ")),
							...(state.cachedLines ?? []).map((line) => prefixLine(line, theme.fg("muted", "  "))),
						];
					}
					return [
						"",
						...(state.cachedLines ?? []).map((line, index) =>
	...
	if (truncation?.truncated || fullOutputPath) {
		...
		component.addChild(
			new Text(
				indentToolBlock(theme.fg("warning", `[${warnings.join(". ")}]`), {
					firstPrefix: theme.fg("warning", "  "),
					restPrefix: theme.fg("warning", "  "),
				}),
				0,
				0,
			),
	...
	if (startedAt !== undefined) {
		...
		component.addChild(
			new Text(
				indentToolBlock(theme.fg("muted", `${label} ${formatDuration(endTime - startedAt)}`), {
...
export function createBashToolDefinition(
	...
	return {
		...
		renderCall(args, _theme, context) {
			...
			text.setText(formatBashCall(args, context.isPartial, context.isError));
			return text;
		},
