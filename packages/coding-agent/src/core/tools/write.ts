...
function formatWriteCall(
	...
	const bullet = getToolStatusBullet(options.isPartial, isError);
	let text = `${bullet} ${theme.fg("toolTitle", theme.bold("Write"))}(${path === null ? invalidArg : path ? theme.fg("accent", path) : theme.fg("toolOutput", "...")})`;
	...
	if (fileContent === null) {
		...
		const displayLines = options.expanded ? lines : lines.slice(-maxLines);
		const renderedBlock = displayLines
			.map((line) => (lang ? line : theme.fg("toolOutput", replaceTabs(line))))
			.join("\n");
		text += `\n${indentToolBlock(renderedBlock)}`;
		if (hiddenLines > 0) {
...
function formatWriteResult(
	...
	return indentToolBlock(theme.fg("error", output), {
		firstPrefix: theme.fg("error", "⎿ "),
		restPrefix: theme.fg("error", "  "),
	});
}
