...
describe("DefaultResourceLoader", () => {
	...
	describe("reload", () => {
		it("should initialize with empty results before reload", () => {
			...
			expect(loader.getCommands().commands).toEqual([]);
...
Skill content here.`,
		...
		it("should discover commands from agentDir", async () => {
			const commandsDir = join(agentDir, "commands");
			mkdirSync(commandsDir, { recursive: true });
			writeFileSync(
				join(commandsDir, "test-prompt.md"),
				`---
...
Prompt content.`,
			...
			expect(commands.some((p) => p.name === "test-prompt")).toBe(true);
		});
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const userPromptPath = join(userCommandsDir, "commit.md");
			const projectPromptPath = join(projectCommandsDir, "commit.md");
			...
			const projectSkillDir = join(cwd, ".claude", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".claude", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".claude", "themes"), { recursive: true });
			...
			const prompt = loader.getCommands().commands.find((p) => p.name === "commit");
		...
		it("should keep both extensions loaded when command names collide", async () => {
			...
			const projectExtDir = join(cwd, ".claude", "extensions");
	...
	pi.registerCommand("user-only", {
		...
		it("should honor overrides for auto-discovered resources", async () => {
			...
			settingsManager.setCommandTemplatePaths(["-commands/skip.md"]);
...
Content`,
			...
			const commandsDir = join(agentDir, "commands");
			mkdirSync(commandsDir, { recursive: true });
			writeFileSync(join(commandsDir, "skip.md"), "Skip prompt");
			...
			expect(commands.some((p) => p.name === "skip")).toBe(false);
		...
		it("should discover SYSTEM.md from cwd/.claude", async () => {
			const piDir = join(cwd, ".claude");
	...
	describe("extendResources", () => {
		it("should load skills and commands with extension metadata", async () => {
...
Extra content`,
			...
			const extraPromptDir = join(tempDir, "extra-commands");
...
Extra prompt content`,
			...
			const { commands } = loader.getCommands();
			const loadedPrompt = commands.find((prompt) => prompt.name === "extra");
