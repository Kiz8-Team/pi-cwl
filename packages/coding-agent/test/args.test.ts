...
describe("parseArgs", () => {
	...
	describe("--command flag", () => {
		test("parses single --command", () => {
			const result = parseArgs(["--command", "./commands"]);
			expect(result.commandTemplates).toEqual(["./commands"]);
		});

		test("parses multiple --command flags", () => {
			const result = parseArgs(["--command", "./one", "--command", "./two"]);
			expect(result.commandTemplates).toEqual(["./one", "./two"]);
		});
	});
	...
	describe("--no-commands flag", () => {
		test("parses --no-commands flag", () => {
			const result = parseArgs(["--no-commands"]);
			expect(result.noCommandTemplates).toBe(true);
		});
	});
