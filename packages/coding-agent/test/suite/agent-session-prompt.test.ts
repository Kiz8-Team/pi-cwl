...
import type { CommandTemplate } from "../../src/core/command-templates.js";
...
describe("AgentSession prompt characterization", () => {
	...
	it("commands while idle and records a single text response", async () => {
	...
	it("expands command templates before sending the prompt", async () => {
		...
		const resourceLoader = {
			...createTestResourceLoader(),
			getCommands: () => ({ commands: [template], diagnostics: [] }),
		};
		const harness = await createHarness({ resourceLoader });
		harnesses.push(harness);
		let expandedPrompt = "";
