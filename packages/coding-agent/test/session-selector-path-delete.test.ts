...
describe("session selector path/delete interactions", () => {
	...
	it("filters empty sessions from the resume list", async () => {
		const sessions = [
			makeSession({ id: "empty", messageCount: 0, firstMessage: "", allMessagesText: "" }),
			makeSession({ id: "non-empty", messageCount: 2, firstMessage: "hello", allMessagesText: "hello world" }),
		];
