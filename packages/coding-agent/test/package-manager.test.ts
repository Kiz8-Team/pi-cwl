...
Content`,
		...
		it("should auto-discover root markdown skills from .claude skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .claude", async () => {
			const extDir = join(tempDir, ".claude", "extensions");
			mkdirSync(extDir, { recursive: true });
		...
		it("should auto-discover user commands with overrides", async () => {
			const commandsDir = join(agentDir, "commands");
			mkdirSync(commandsDir, { recursive: true });
			const promptPath = join(commandsDir, "auto.md");
			writeFileSync(promptPath, "Auto prompt");

			settingsManager.setCommandTemplatePaths(["!commands/auto.md"]);

			const result = await packageManager.resolve();
			expect(result.commands.some((r) => r.path === promptPath && !r.enabled)).toBe(true);
		});

		it("should auto-discover project commands with overrides", async () => {
			const commandsDir = join(tempDir, ".claude", "commands");
			mkdirSync(commandsDir, { recursive: true });
			const promptPath = join(commandsDir, "is.md");
			writeFileSync(promptPath, "Is prompt");

			settingsManager.setProjectCommandTemplatePaths(["!commands/is.md"]);
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".claude", "agent");
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .claude auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".claude\n");

			const skillDir = join(tempDir, ".claude", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .claude settings base", () => {
			...
			const rel = relative(join(tempDir, ".claude"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("HTTPS git URL parsing (old behavior)", () => {
		it("should parse HTTPS GitHub URLs correctly", async () => {
			...
			expect(parsed.claudenned).toBe(false);
		});
		...
		it("should parse HTTPS URLs with ref", async () => {
			...
			expect(parsed.claudenned).toBe(true);
		});
	...
	describe("pattern filtering in top-level arrays", () => {
		...
		it("should filter commands with exclusion pattern", async () => {
			const commandsDir = join(agentDir, "commands");
			mkdirSync(commandsDir, { recursive: true });
			writeFileSync(join(commandsDir, "review.md"), "Review code");
			writeFileSync(join(commandsDir, "explain.md"), "Explain code");

			settingsManager.setCommandTemplatePaths(["commands", "!explain.md"]);
	...
	describe("force-include patterns", () => {
		...
		it("should force-include commands", async () => {
			const commandsDir = join(agentDir, "commands");
			mkdirSync(commandsDir, { recursive: true });
			writeFileSync(join(commandsDir, "review.md"), "Review");
			writeFileSync(join(commandsDir, "explain.md"), "Explain");
			writeFileSync(join(commandsDir, "debug.md"), "Debug");

			settingsManager.setCommandTemplatePaths(["commands", "!commands/*.md", "+commands/debug.md"]);
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".claude", "npm")],
				undefined,
			);
		});
		...
		it("should skip installing missing package sources when offline", async () => {
			...
			const result = await packageManager.resolve();
			const allResources = [...result.extensions, ...result.skills, ...result.commands, ...result.themes];
		...
		it("should not fetch npm registry during resolve for installed unpinned packages", async () => {
			const installedPath = join(tempDir, ".claude", "npm", "node_modules", "example");
			mkdirSync(join(installedPath, "extensions"), { recursive: true });
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".claude", "npm", "node_modules", "example");
