...
**Input expansion**: Skill commands (`/skill:name`) and command templates (`/template`) are expanded before sending/queueing.
...
Queue a steering message while the agent is running. It is delivered after the current assistant turn finishes executing its tool calls, before the next LLM call. Skill commands and command templates are expanded. Extension commands are not allowed (use `prompt` instead).
...
Queue a follow-up message to be processed after the agent finishes. Delivered only when agent has no more tool calls or steering messages. Skill commands and command templates are expanded. Extension commands are not allowed (use `prompt` instead).
...
Get available commands (extension commands, command templates, and skills). These can be invoked via the `prompt` command by prefixing with `/`.
...
```json
{
  ...
  "data": {
    "commands": [
      {"name": "session-name", "description": "Set or clear session name", "source": "extension", "path": "/home/user/.claude/agent/extensions/session.ts"},
      {"name": "fix-tests", "description": "Fix failing tests", "source": "prompt", "location": "project", "path": "/home/user/myproject/.claude/agent/commands/fix-tests.md"},
      {"name": "skill:brave-search", "description": "Web search via Brave API", "source": "skill", "location": "user", "path": "/home/user/.claude/agent/skills/brave-search/SKILL.md"}
    ]
  }
}
```
...
- `source`: What kind of command:
  ...
  - `"prompt"`: Loaded from a command template `.md` file
  - `"skill"`: Loaded from a skill directory (name is prefixed with `skill:`)
- `location`: Where it was loaded from (optional, not present for extensions):
  - `"user"`: User-level (`~/.claude/agent/`)
  - `"project"`: Project-level (`./.claude/agent/`)
