...
> **Placement for /reload:** Put extensions in `~/.claude/agent/extensions/` (global) or `.claude/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.claude/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.claude/agent/extensions/*.ts` | Global (all projects) |
| `~/.claude/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.claude/extensions/*.ts` | Project-local |
| `.claude/extensions/*/index.ts` | Project-local (subdirectory) |
...
4. If not handled: command templates (`/template`) expanded to template content
...
- **Print mode:** No-op. The process exits automatically when all commands are processed.
...
pi.registerCommand("reload-runtime", {
  description: "Reload extensions, skills, commands, and themes",
...
Get the slash commands available for invocation via `prompt` in the current session. Includes extension commands, command templates, and skill commands.
...
```typescript
{
  ...
  description?: string;
  source: "extension" | "command" | "skill";
  sourceInfo: {
    path: string;
    source: string;
