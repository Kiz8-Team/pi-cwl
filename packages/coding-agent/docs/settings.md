...
| `.claude/settings.json` | Project (current directory) |
...
Paths in `~/.claude/agent/settings.json` resolve relative to `~/.claude/agent`. Paths in `.claude/settings.json` resolve relative to `.pi`. Absolute paths and `~` are supported.
...
Project settings (`.claude/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.claude/agent/settings.json (global)
{
  "theme": "dark",
