...
```
<cwd>/.pi/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```

Where `<cwd>` is the working directory where pi was opened, and `<path>` is that working directory with `/` replaced by `-`.

## Deleting Sessions

Sessions can be removed by deleting their `.jsonl` files under `<cwd>/.pi/agent/sessions/`.

## Passive CWL JSON Traces

Pi also writes passive Context Window Lifecycle (CWL) JSON traces for persisted sessions under:

```text
<cwd>/.pi/agent/cwl/*.json
```

These traces mirror the opencode-style CWL viewer schema and are updated automatically as the session evolves.

A bundled Python viewer is available at:

```text
packages/coding-agent/scripts/cwl-view.py
# or in published builds:
<package>/dist/scripts/cwl-view.py
```

Example:

```bash
python3 packages/coding-agent/scripts/cwl-view.py
python3 packages/coding-agent/scripts/cwl-view.py <cwd>/.pi/agent/cwl/<trace>.json
```
