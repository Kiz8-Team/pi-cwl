...
`createAgentSession()` uses a `ResourceLoader` to supply extensions, skills, command templates, themes, and context files. If you do not provide one, it uses `DefaultResourceLoader` with standard discovery.
...
const { session } = await createAgentSession({
  model: myModel,
  cwlLimit: { type: "tokens", value: 120_000 },
...
The `prompt()` method handles command templates, extension commands, and message sending:
...
- **File-based command templates** (from `.md` files): Expanded to their content before sending/queueing.
...
Both `steer()` and `followUp()` expand file-based command templates but error on extension commands (extension commands cannot be queued).
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.claude/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.claude/extensions/`)
- Project skills:
  - `.claude/skills/`
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.claude/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.claude/agent/auth.json and ~/.claude/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.claude/agent/extensions/`, `.claude/extensions/`, and settings.json extension sources.
...
const customCommand: CommandTemplate = {
  name: "deploy",
...
const loader = new DefaultResourceLoader({
  commandsOverride: (current) => ({
    commands: [...current.commands, customCommand],
    diagnostics: current.diagnostics,
  }),
});
await loader.reload();
...
> See [examples/sdk/08-commands.ts](../examples/sdk/08-commands.ts)
...
const { session } = await createAgentSession({
  settingsManager,
  cwlLimit: { type: "percent", value: 60 },
});
...
1. Global: `~/.claude/agent/settings.json`
...
Single-shot mode: send commands, output result, exit:
